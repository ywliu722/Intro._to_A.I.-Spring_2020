...
void sending_pkt(char *src_ip, int src_port, char *dns_ip, int dns_port, unsigned char *query_site){
    ...
    ip->ihl = 5;
    ip->tos = 0;
    ip->tot_len = sizeof(ip_header)+sizeof(udp_header)+sizeof(dns_header)+(strlen(query_site)+2)+sizeof(question);
    ip->id = htonl(getpid());
    ip->frag_off = 0;
    ip->ttl = 64;
    ...
    udp->len = htons(sizeof(udp_header)+sizeof(dns_header)+(strlen(query_site)+2)+sizeof(question));
    udp->check = 0;
    ...
    ps.len = htons(sizeof(udp_header)+sizeof(dns_header)+(strlen(query_site)+2)+sizeof(question));
    int siz=sizeof(pseudo)+sizeof(udp_header)+sizeof(dns_header)+(strlen(query_site)+2)+sizeof(question);
    
    pseudo_header_buffer=malloc(siz);
    
    memcpy(pseudo_header_buffer, (unsigned char*)&ps, sizeof(pseudo));
    memcpy(pseudo_header_buffer+sizeof(pseudo), udp, sizeof(udp_header)+sizeof(dns_header)+(strlen(site_trans)+2)+sizeof(question));
